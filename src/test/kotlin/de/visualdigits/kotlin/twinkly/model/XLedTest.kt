...
class XLedTest {
    ...
    fun testRealTime() {
        ...
        File(ClassLoader.getSystemResource("images/tetris").toURI())
            ...
            ?.forEach { file ->
                xled.showRealTimeFrame(XledFrame.fromImage(xled.columns, xled.rows, xled.bytesPerLed, file))
                Thread.sleep(20)
            }
    }
    ...
    @Test
    fun testImage() {
        xled.showFrame("Test", XledFrame.fromImage(xled.columns, xled.rows, xled.bytesPerLed, File(ClassLoader.getSystemResource("images/test-image.png").toURI())))
    }
